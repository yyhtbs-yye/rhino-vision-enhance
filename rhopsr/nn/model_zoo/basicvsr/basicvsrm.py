...
from .basicvsrpp import ConvResidualBlocks, Upsampler

class BasicVSRMinus(nn.Module):
    ...
    def propagate(self, feats, flows, module_name):
        ...
        feat_prop2 = flows.new_zeros(n, self.mid_channels, h, w)

        for i, idx in enumerate(frame_idx):
            feat_current = feats['spatial'][mapping_idx[idx]]

            # default aligned features: just current propagated state
            feat_prop1_align = feat_prop1
            feat_prop2_align = feat_prop2
            ...
            if i > 0:
                ...
                if i > 1:  # second-order features
                    ...
                    feat_prop2_align = flow_warp(feat_prop2, flow_n2.permute(0, 2, 3, 1))

            # concatenate and residual blocks
            feat = [feat_current, feat_prop1_align, feat_prop2_align] + [feats[k][idx] for k in feats if k not in ['spatial', module_name]]

            # second-order state: previous first-order state
            feat_prop2 = feat_prop1

            feat = torch.cat(feat, dim=1)
            feat_prop1 = feat_prop1 + self.backbone[module_name](feat)
